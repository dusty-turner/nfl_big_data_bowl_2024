---
title: "Quantifying Tackeling in Football"
...
* Penalized Regression: {GLMNET}
    + Train: `r pen_rds$nrow_train`
    + Test: `r pen_rds$nrow_test`
    + Validate: `r pen_rds$nrow_val`
    + Baseline Accuracy: `r rf_rds$baseline_accuracy`
* Random Forest: {Ranger}
    + Train: `r rf_rds$nrow_train`
    + Test: `r rf_rds$nrow_test`
    + Validate: `r rf_rds$nrow_val`
    + Baseline Accuracy: `r rf_rds$baseline_accuracy`
...
* XGBoost: {XGBoost}
    + Train: `r xg_rds$nrow_train`
    + Test: `r xg_rds$nrow_test`
    + Validate: `r xg_rds$nrow_val`
    + Baseline Accuracy: `r rf_rds$baseline_accuracy`
* Neural Network: {Reticulate} (Python Tensor Flow)
    + Train: `r #nn_rds$nrow_train`
    + Test: `r #nn_rds$nrow_test`
    + Validate: `r #nn_rds$nrow_val`
...
The best parameters are: Lambda = `r round(as.numeric(pen_rds$best_parameters$penalty), 5)` and Alpha = `r as.character(round(as.numeric(pen_rds$best_parameters$mixture), 7))` with an accuracy of `r stringr::str_c(round(pen_rds$best_parameters$mean,4)*100, "%")`

<center>

```{r}
pen_rds$pen_grid_results +
  geom_point(aes(x = pen_rds$best_parameters$penalty, y = pen_rds$best_parameters$penalty), shape = 4, color = "black", size = 5)
```
...
The best parameters are: Mtry = `r round(as.numeric(rf_rds$best_parameters$mtry), 1)`, Min_n = `r round(as.numeric(rf_rds$best_parameters$min_n), 1)`, and Trees = `r round(as.numeric(rf_rds$best_parameters$trees), 1)` with an accuracy of `r stringr::str_c(round(as.numeric(rf_rds$rf_accuracy$.estimate[1]),4)*100, "%")`.

```{r}
rf_rds$rf_grid_results +
    geom_point(aes(x = rf_rds$best_parameters$min_n, y = rf_rds$best_parameters$mtry), shape = 4, color = "black")
```
...
The best parameters are: Trees = `r round(as.numeric(xg_rds$best_parameters$trees), 1)`, Min_n = `r round(as.numeric(xg_rds$best_parameters$min_n), 1)`, Tree Depth = `r round(as.numeric(xg_rds$best_parameters$tree_depth), 1)`, Learn Rate = `r round(as.numeric(xg_rds$best_parameters$learn_rate), 1)`, Loss Reduction = `r round(as.numeric(xg_rds$best_parameters$loss_reduction), 0)`, and Sample Size = `r round(as.numeric(xg_rds$best_parameters$sample_size), 0)` with an accuracy of `r stringr::str_c(round(as.numeric(rf_rds$rf_accuracy$.estimate[1]),4)*100, "%")`.
...
make_gt <-  function(data, title, acc = nn_rds$accuracy){
  acc <- str_c(round(acc,4)*100,"%")
  data |>
  arrange(-tackles_over_expected) |> 
    rename(`Display Name` = display_name,  `Tackles Over Expected` = tackles_over_expected) |> 
  slice(c(1:4,(n() - 3):n())) |> 
  select(-nfl_id) |> 
  gt::gt() |> gt::fmt_number() |>  
    gt::tab_header(title = title,subtitle = str_c("Accuracy: ", acc))
}
```
...
```{r}
pen_rds$pen_briar |> make_gt(title = "Penalized Regression", acc = pen_rds$best_parameters$mean)
```
...
```{r}
rf_rds$rf_briar |> make_gt(title = "Penalized Regression", acc = rf_rds$best_parameters$mean)
```
:::
...
```{r}
xg_rds$xg_briar |> make_gt(title = "Penalized Regression", acc = xg_rds$best_parameters$mean)
```
