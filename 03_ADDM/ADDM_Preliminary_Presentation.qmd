...
week_1 |> 
  ...
  filter(defensiveTeam == club | is_football == "football") |> 
  mutate(color = ifelse(is_football != "football", distance_to_ball, NA)) |> 
  select(displayName, x, y, x_ball, y_ball, v_approach, color, absoluteYardlineNumber, ball_carrier, playId, time, is_football, club, distance_to_ball, playDescription, x_facing, y_facing,o, jerseyNumber) %>% 
{
  ggplot(data = ., aes(x = x, y = y)) +
  geom_vline(aes(xintercept = absoluteYardlineNumber), color = "blue") +
  geom_point(aes(shape = is_football), show.legend = FALSE, size = 5) +
  # geom_text(aes(label = jerseyNumber), color = "red", nudge_x = 0, nudge_y = -1) +
  geom_segment(aes(xend = x_facing, yend = y_facing), arrow = arrow(length = unit(0.03, "npc")), color = "black") +
...
week_1 |>  
  filter(gameId == dak$gameId[2], playId %in% unique(dak$playId)[2]) |> 
  filter(defensiveTeam == club | is_football == "football") |> 
        select(frameId, x, y, is_football, time, oriented_towards_ball, oriented_towards_ball, turning_towards, new_x, new_y, contains("fan"), absoluteYardlineNumber, jerseyNumber, is_football, playDescription) %>% 
{
  ggplot(data = ., aes(x = x, y = y, color = oriented_towards_ball, shape = is_football)) +
  geom_vline(aes(xintercept = absoluteYardlineNumber), color = "blue") +
  ...
  # labs(y = "", x = "Yards To Line of Scrimmage", title = "str_wrap(.$playDescription[1], width = 80)") +
  labs(y = "", x = "Yards To Line of Scrimage", title = str_wrap(.$playDescription[1], width = 80)) +
       # title = 'FrameId: {closest_state}') +
...
week_1 |> 
  ...
  filter(defensiveTeam == club | is_football == "football") |> 
  mutate(color = ifelse(is_football != "football", distance_to_ball, NA)) |>
  select(displayName, x, y, new_x, new_y, v_approach, color, absoluteYardlineNumber, ball_carrier, playId, time, is_football, club, distance_to_ball, playDescription, x_facing, y_facing,o, jerseyNumber) %>% 
{
  ggplot(data = ., aes(x = x, y = y)) +
  geom_vline(aes(xintercept = absoluteYardlineNumber), color = "blue") +
  geom_point(aes(shape = is_football), show.legend = FALSE, size = 5) +
...
week_1 |> 
  ...
  filter(defensiveTeam == club | is_football == "football") |> 
  mutate(color = ifelse(is_football != "football", distance_to_ball, NA)) |> 
  select(displayName, x, y, x_ball, y_ball, v_approach, color, absoluteYardlineNumber, ball_carrier, playId, time, is_football, club, distance_to_ball, playDescription, x_facing, y_facing, x_ball_next, y_ball_next) %>% 
  # filter(is_football == "football") |> select(x, y, x_facing, y_facing)
{
...
<!--   filter(defensiveTeam == club | is_football == "football") |>  -->
<!--   mutate(color = ifelse(is_football != "football", distance_to_ball, NA)) |>  -->
<!--   select(gameId, displayName, x, y, v_approach, color, absoluteYardlineNumber, ball_carrier, playId, time, is_football, club, distance_to_ball, playDescription, x_facing, y_facing, alignment) %>%  -->
...
defensive_model_building_data |>
  ...
  filter(defensiveTeam == club | is_football == "football") |>
  mutate(color = ifelse(is_football != "football", distance_to_ball, NA)) |>
  select(gameId, displayName, x, y, v_approach, color, absoluteYardlineNumber, ball_carrier, playId, time, is_football, club, distance_to_ball, playDescription, x_facing, y_facing, position) %>%
...
defensive_model_building_data |> 
  ...
  select(gameId, displayName, x, x_from_los, y, v_approach, absoluteYardlineNumber, ball_carrier, playId, time, is_football, club, distance_to_ball, playDescription, x_facing, y_facing, alignment, alignment_cluster) %>%
