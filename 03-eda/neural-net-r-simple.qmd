...
y_pred_binarized <- array(ifelse(y_pred > 0.5, 1, 0), dim(y_pred_test))
...
y_pred_test_flat <- lapply(seq_len(dim(y_pred)[1]), function(i) {
  t(y_pred[i,,])
})
...
roc_obj <- roc(response = test_joined_with_preds$tackle, predictor = test_joined_with_preds$predictions)
