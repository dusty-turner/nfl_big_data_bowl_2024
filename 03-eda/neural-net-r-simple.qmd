...
center_and_scale <- c("x", "y", "distance_to_ball", "distance_to_ball_next", "x_going", "y_going", "s", "a", "o", "dir" ,"x_ball",
                      "y_ball", "x_ball_next", "y_ball_next", "s_ball", "o_ball", "dir_ball", "angle_to_ball", "pass_probability", "yards_to_go",
                      "x_from_los", "height", "weight")
...
newdata <-
  ...
  mutate(across(.cols = any_of(center_and_scale), .fns = ~(.-mean(., na.rm = TRUE))/sd(., na.rm = TRUE))) |> 
...
newdata <-
newdata |> select(game_idplay_id,game_id, play_id, frame_id, tackle, x, y, distance_to_ball, x_ball, y_ball, contains("position"))
...
# Assuming `max_length` is the length of your longest sequence
max_length <- max(newdata$frame_id)  

# Preprocess your data
# Pad your sequences here. You might need to reshape your data appropriately.
train_padded <- pad_sequences(train_data, maxlen = max_length)
test_padded <- pad_sequences(test_data, maxlen = max_length)

# Define your LSTM model
model <- keras_model_sequential() %>%
  layer_lstm(units = 50, input_shape = c(max_length, ncol(train_data) - 1)) %>%  # Adjust the input_shape based on your data
  layer_dense(units = 1, activation = 'sigmoid')  # Assuming binary classification for `tackle`
...
model %>% fit(
  train_padded,
  train_data$tackle,  # Your target variable
  epochs = 10,       # Number of epochs
  batch_size = 32    # Batch size, adjust based on your dataset size
)

# Evaluate the model
model %>% evaluate(test_padded, test_data$tackle)
