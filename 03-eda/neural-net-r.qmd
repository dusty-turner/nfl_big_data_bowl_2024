...
#   layer_lstm(units = 100, return_sequences = TRUE, input_shape = c(N, num_features)) %>%
...
custom_loss <- function(y_true, y_pred) {
  ...
  lambda <- .9  # This is a hyperparameter you can tune
  return(bce + lambda * sum_constraint)
}


num_players <- 22  # Assuming 22 players in each frame, adjust based on your data
N <- ncol(x_train)
num_features <- ncol(list_of_sequences[[1]])
...
# Group data by game_id, play_id, and frame_id
get_custom_batches <- function(grouped_data, batch_size, max_length = 204, num_features) {
  batches <- list()
  batch_x <- list()
  batch_y <- list()
  ...
  for (i in seq_len(nrow(grouped_data))) {
    ...
    group_x <- as.matrix(group[, setdiff(names(group), "tackle"), with = FALSE])
    group_y <- group$tackle
    ...
    # Add the resized group data to the batch lists
    batch_x <- c(batch_x, list(resized_group_x))
    batch_y <- c(batch_y, list(group_y))

    # Check if the current batch has enough sequences
    if (length(batch_x) >= batch_size) {
      ...
      batches[[length(batches) + 1]] <- list(x = combined_x, y = combined_y)
      batch_x <- list()
      batch_y <- list()
    }
  }
...
# Example usage
batch_size <- 220  # Define your batch size
max_length <- 204  # Correct sequence length
...
# get_batches <- function(x, y, batch_size) {
#   n <- dim(x)[1]  # Assuming the first dimension is the number of samples
#   perm <- sample(n)
#   
...
#     x_batch <- x[idx,,]
#     y_batch <- y[idx]
#     list_of_batches[[length(list_of_batches) + 1]] <- list(x = x_batch, y = y_batch)
#   }
# 
#   list_of_batches
# }
...
# Custom training loop with a progress bar
# for (epoch in 1:num_epochs) {
#   tic()  # Start timing the epoch
#   cat("Epoch", epoch, "/", num_epochs, "\n")
#   
#   total_loss <- 0
#   num_batches <- length(get_batches(x_train, y_train, batch_size))
#   
#   # Create a progress bar
#   pb <- progress_bar$new(
#     format = "  [:bar] :percent :current/:total (eta: :eta)",
...
#   )
# 
#   for (i in seq_along(get_batches(x_train, y_train, batch_size))) {
#     batch <- get_batches(x_train, y_train, batch_size)[[i]]
#     tape <- tf$GradientTape()
#     
#     with(tape, {
#       predictions <- model(batch$x)
...
#   # Add your validation evaluation code here (optional)
# }
