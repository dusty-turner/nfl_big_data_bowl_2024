...
### use this to somehow add back in tackles that were not included

# week_1 |> 
#   filter(game_id == 2022090800, play_id == 146) |> 
#   filter(str_detect(display_name, "Wagner")) |> 
#   filter(is.na(pass_result)) |>
#   group_by(game_id, play_id) |> 
#   # filter(cur_group_id() == 1) |> 
#   select(display_name, play_description, tackle) |> 
#    mutate(names_in_description = str_extract(play_description, "\\([^()]*\\)(?!.*\\()")) |> 
#     
#     # Check if display_name is in the extracted names
#     mutate(names_in_description = str_replace_all(names_in_description, "[()]", "")) %>%
#   mutate(
#     # Convert logical to numeric
#     tackle = as.numeric(str_detect(names_in_description, fixed(display_name)))
#   ) 
#   filter(tackle == 1)
#   # mutate(tackle = ifelse(str_detect(play_description)))
# mutate(tackle = as.numeric(as.character(tackle))) |> 
# group_by(game_id, play_id, display_name) |>
# reframe(test = mean(as.numeric(tackle))) |> 
# group_by(game_id, play_id) |> 
...
# continuous_factors <- c("x", "y", "x_ball", "y_ball", "s", "a", "o", "distance_to_ball")
...
wrong <-
  newdata |> group_by(game_id, play_id, display_name) |> 
  reframe(test = mean(tackle)) |> 
  group_by(game_id, play_id) |> 
  reframe(sum = sum(test))  |> 
  filter(sum != 1) |> 
...
## currently removed plays with no tackle
newdata <- 
  newdata |> 
  # filter(game_id == 2022090800, play_id == 146) |>
  mutate(temp = str_c(game_id, "_", play_id)) |> 
  # select(temp) |> 
  filter(!temp %in% wrong$temp) |> 
  select(-temp)
```
...
#   time_distributed(layer_dense(units = 11, activation = 'sigmoid'))  # Adjust the unit count based on output shape
    ...
    model <- keras_model_sequential() %>%
  layer_masking(mask_value = 0, input_shape = c(max_length, num_features)) %>%
  layer_lstm(units = units_param, return_sequences = TRUE,
             # recurrent_activation = 'relu' ## this made it give NANs for the loss
             kernel_regularizer = regularizer_l2(l2_value_param),
             recurrent_regularizer = regularizer_l2(l2_value_param)
             ) %>%
...
history <- model %>% fit(
  x = x_train, y = y_train,
  epochs = epochs_param,
  batch_size = batch_size_param,  # this is how many sequences go at once
...
accuracy <- sum(array(ifelse(predict(model, x_test) > 0.5, 1, 0), dim(predict(model, x_test))) == y_test) / length(y_test)

  tibble(accuracy = accuracy, epochs = epochs_param, units = units_param, layers = layers_param, 
         l2_value = l2_value_param,
         optimizer = optimizer_param, rate = rate_param, time = now(), 
         user = Sys.info()["user"], positive_weight_param = positive_weight_param,
         # activation = activation_param, 
         model_execution_time = model_execution_time, notes = notes_param) |> 
...
out <- run_nn(
  ...
  l2_value_param = .001,
  layers_param = 3,
  batch_size_param = 441/1,
  notes_param = "using 1 week with new metric",
  positive_weight_param = 11,
  optimizer_param = "adam"
  # activation_param = "relu"
)

epoch_values <- c(100, 250, 1000)
rate_values <- c(.001, .005, .01)
l2_values <- c(.01, .05, .1)
positive_weight_value <- c(3, 7, 11)
<<<<<<< HEAD
units_values <- c(50, 75, 100)
=======
units_value <- c(50,75,100)
>>>>>>> fc59b6e80cbf7b7999962153a04b90687a2edf4d
...
pmap(param_grid, function(epochs_param, rate_param, l2_value_param, units_param, positive_weight_param) {
  run_nn(
    ...
    layers_param = 3,  # Assuming a fixed value
    batch_size_param = 441/1,  # Assuming a fixed value
    notes_param = "Hyperparameter tuning",
