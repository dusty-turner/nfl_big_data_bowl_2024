...
defensive_model_building_data %>% 
  ...
  select(c(x,y,distance_to_ball, distance_to_ball_next, x_going, y_going, s, a, o, dir, x_ball, y_ball, x_ball_next, y_ball_next, s_ball, o_ball, dir_ball, angle_to_ball,
           position, offense_formation, quarter, down, rank, alignment_cluster, 
...
should_be_factors <- c("ball_in_fan3", "ball_in_fan2", "ball_in_fan1", "position", "offense_formation", "quarter", "down", "rank", "defenders_in_the_box", "alignment_cluster")
...
discrete_factors <- c("quarter", "position", "offense_formation", "down", "rank", "defenders_in_the_box", 
                      "ball_in_fan3", "ball_in_fan2", "ball_in_fan1", "alignment_cluster")
...
out <- run_nn(
  ...
  # positive_weight_param = ratio,
  notes_param = "aadd in cluster 2 layers 1000 epoch - the last one had weird behavior at 450",
...
```{r}
save_model_tf(out$model, str_c("04-models"))

model2 <- load_model_tf("04-models/2023-12-31_03-07-53", custom_objects = list(
...
```{r}
write_rds(test_joined_with_preds,"04-models/test_joined_with_preds.rds")
```
...
for (i in 1:length(x_padded_1)) {
  x_data_1[i, , ] <- x_padded[[i]]
}

y_data_1 <- array(dim = c(length(y_padded_1), max_length, ncol(y_padded[[1]])))
...
test_joined_with_preds_1 |> 
  # select(game_idplay_id, display_name, frame_id, tackle,  final_predictions_1, binary_predictions_1) |> 
  select(game_idplay_id, display_name, frame_id, tackle, final_predictions_1) |> 
  # left_join(test_joined_with_preds |> select(game_idplay_id, display_name, frame_id, final_predictions, binary_predictions)) |> 
  ...
  group_by(display_name) |> 
  summarise(total_probability_added = sum(increase_in_tackle_prob), average_probability_added = mean(increase_in_tackle_prob)) |>
  ungroup() |> 
  arrange(-total_probability_added) 

}

# map_dfr(.x = vectors[1:2], .f = ~double_creater(player_number = 1, vec = .x), .id = "vector_id")
player_out_results <-
map2_dfr(.x = sort(rep(1:10, 10)), .y = rep(vectors,10), .f = ~double_creater(player_number = .x, vec = .y), .id = "vector_id")

player_out_results |> 
  group_by(display_name) |> 
  summarise(total_probability_added = sum(total_probability_added), average_probability_added = mean(average_probability_added)) |> 
  arrange(total_probability_added)
