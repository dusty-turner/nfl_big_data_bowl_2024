...
```{r}
units <- 100
rate <- 0.4  # Increased dropout rate
l2_value <- 0.001  # L2 regularization factor
epochs <- 600
...
model <- keras_model_sequential() %>%
  ...
  bidirectional(layer_lstm(units = units, return_sequences = TRUE, activation = 'relu', 
                           recurrent_activation = 'sigmoid',
                           kernel_regularizer = regularizer_l2(l2_value),
                           recurrent_regularizer = regularizer_l2(l2_value))) %>%
  ...
  time_distributed(layer_dense(units = 11, activation = 'sigmoid', 
                               kernel_regularizer = regularizer_l2(l2_value)))  # Sigmoid for classification
...
tibble(accuracy = accuracy, epochs = epochs, units = units, layers = 3, rate = rate, time = now(), user = Sys.info()["user"], activation = "relu") |> 
  bind_rows(read_csv("model_tracker.csv")) |> 
  write_csv("model_tracker.csv")
