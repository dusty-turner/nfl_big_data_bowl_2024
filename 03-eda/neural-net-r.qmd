...
if (Sys.info()["user"] == "dusty_turner1") {
  write_rds(list(x_train = x_train, y_train = y_train, x_test = x_test, y_test = y_test, test_data = test_data, max_length = max_length), "model_data.rds")
}
```
...
out <- run_nn(
  ...
  l2_value_param = .001,
  layers_param = 4,
  batch_size_param = 3994,
  positive_weight_param = ratio,
  notes_param = "best from grid search with more data",
  optimizer_param = "adam",
  # activation_param = "relu"
git)

epoch_values <- c(1000)
rate_values <- c(.05,.1,.15)
l2_values <- c(.001)
positive_weight_value <- c(ratio)
units_value <- c(100,150,200)
...
pmap(param_grid, function(epochs_param, rate_param, l2_value_param, units_param, positive_weight_param) {
  run_nn(
    ...
    layers_param = 4,  # Assuming a fixed value
    batch_size_param = 3994,  # Assuming a fixed value
...
list.files() %>% as_tibble() %>% filter(str_detect(value, "gganim_plot")) %>% pull() %>% file.remove()
