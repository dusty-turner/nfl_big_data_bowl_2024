...
```{r}
run_nn <- function(units_param = 50, rate_param = .5, l2_value_param = .01, epochs_param = 10, activation_param = "relu", optimizer_param = 'adam', layers_param = 3, batch_size_param = 441/1, notes_param = NULL){
  ...
  raw_tracker <-
    ...
    raw_tracker |> 
    filter(epochs == epochs_param, units == units_param, rate == rate_param, l2_value == l2_value_param, activation == activation_param,optimizer == optimizer_param, layers == layers_param) %>% 
    print()
...
model <- keras_model_sequential() %>%
  ...
  bidirectional(layer_lstm(units = units_param, return_sequences = TRUE, activation = activation_param, 
  ...
  time_distributed(layer_dense(units = 11, activation = 'sigmoid', 
                               kernel_regularizer = regularizer_l2(l2_value_param)))  # Sigmoid for classification
...
model_execution_time <- model_end_time - model_start_time  # Calculate execution time for model building and training
model_execution_time |> str()
...
accuracy <- sum(array(ifelse(predict(model, x_test) > 0.5, 1, 0), dim(predict(model, x_test))) == y_test) / length(y_test)
  ...
  tibble(accuracy = accuracy, epochs = epochs_param, units = units_param, layers = layers_param, 
         l2_value = l2_value_param, optimizer = optimizer_param, rate = rate_param, time = now(), 
         user = Sys.info()["user"], activation = activation_param, model_execution_time = model_execution_time, notes = notes_param) |> 
