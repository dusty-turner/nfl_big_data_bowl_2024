...
sequence_length <- dim(x_train)[2]  # This extracts the number of time steps (sequence length)
...
# Example usage
# Example batch size (you can adjust this based on your needs and computational resources)
batch_size <- 1  # or any other appropriate number

# Get max_length from the padded training data
max_length <- dim(x_train)[2]

# Get num_features from the padded training data (or from the list of sequences)
num_features <- dim(x_train)[3]

# Now call the function with these dynamic variables
custom_batches <- get_custom_batches(grouped_data, batch_size, max_length, num_features)

custom_batches[[1]]$display_names
# |> as_tibble() 
# |> count(display_name)
...
# Initialize a list to store processed predictions
...
for (i in seq_along(list_of_test_sequences)) {
  ...
  # Truncate the predictions to the original length of the sequence
  processed_predictions[[i]] <- predictions[i, 1:original_length, 1]
}

# Flatten the list of predictions to a single vector
flat_predictions <- unlist(processed_predictions)

# Adjust y_test to match the structure of flat_predictions
# Assuming y_test is already a vector with one label per time step
y_test_adjusted <- unlist(lapply(seq_along(list_of_test_sequences), function(i) {
  y_test[sum(sapply(list_of_test_sequences[1:(i-1)], nrow)) + 1 : nrow(list_of_test_sequences[[i]])]
}))

# Apply threshold if needed (for binary classification)
threshold <- 0.5
binary_predictions <- ifelse(flat_predictions > threshold, 1, 0)

# Calculate accuracy
accuracy <- sum(binary_predictions == y_test_adjusted) / length(y_test_adjusted)
...
# Assuming flat_predictions and y_test_adjusted are as defined previously
roc_result <- roc(y_test_adjusted, flat_predictions)
...
# Flatten the list of predictions to a single vector (already done in flat_predictions)

# Assuming test_data is in the same order as the predictions
test_set_with_predictions <- 
  test_data %>%
  mutate(prediction = flat_predictions) %>% 
  mutate(binary_predictions = as.factor(binary_predictions)) |> 
  as_tibble() %>% 
  mutate(tackle = as.factor(tackle)) %>% 
  relocate(game_id, play_id, prediction, binary_predictions, frames_from_tackle, tackle) %>% 
  mutate(across(.cols = c(game_id, play_id, nfl_id), .fns = ~as.character(.)))

# Histogram of Predictions
hist(flat_predictions)
