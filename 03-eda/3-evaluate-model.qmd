...
library(yardstick)
# Define the multiple classification metrics
classification_metrics <- metric_set(accuracy, precision, recall)


fake <- factor(0, levels = c(0,1))

test_joined_with_preds %>%
  mutate(fake_prediction = fake) |> 
  # filter(frames_from_tackle >= -40) %>%
  # group_by(frames_from_tackle, play_type) %>%
  classification_metrics(truth = tackle, estimate = fake_prediction)

# Calculate the metrics for your data
accuracy_plot <-
test_joined_with_preds %>%
  filter(frames_from_tackle >= -40) %>%
  group_by(frames_from_tackle, play_type) %>%
  classification_metrics(truth = tackle, estimate = binary_predictions) |>
  mutate(.metric = str_to_title(.metric)) |> 
  mutate(play_type = str_to_sentence(play_type)) |>
  mutate(play_type = fct_relevel(play_type, "Run")) |> 
  ggplot(aes(x = frames_from_tackle, y = .estimate, alpha = frames_from_tackle)) +
  geom_point() +
  geom_line() +
  scale_y_continuous(labels = scales::percent_format(accuracy = 0.1, scale = 100)) +
  scale_x_continuous(labels = c(4,3,2,1,0), breaks = c(-40,-30,-20,-10,0)) +
  facet_grid(.metric~play_type, scales = "free_y") +
  # facet_grid(play_type~.metric, nrow = 3, scales = "free_y") +
  labs(y = "", x = "Seconds From Tackle") +
  theme_minimal(base_size = 13) +
  theme(legend.position = "none", panel.grid.minor = element_blank(), 
        panel.background = element_rect(color = "grey75"), 
        plot.background = element_rect(fill = "white", color = NA)) 

ggsave("05-presentation-files/accuracy-plot-1000-400.svg", plot = accuracy_plot, width = 9, height = 6)
...
week_1 %>% filter(game_idplay_id == "20220911001587") |> filter(tackle == 1) |> select(display_name, jersey_number)
...
animated_plot <-
  ...
  week_1 %>% filter(game_idplay_id == "20220911001587") |> 
...
week_1 |> 
  ...
  filter(game_idplay_id == "20220911001587") |>
