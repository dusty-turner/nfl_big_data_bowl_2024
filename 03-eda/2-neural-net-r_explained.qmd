...
tictoc::tic()
player_out_results <-
future_map2_dfr(.x = sort(rep(1:10, 10)), .y = rep(vectors,10), .f = ~double_creater(player_number = .x, vec = .y), .id = "vector_id", .progress = TRUE)
tictoc::toc()

# player_out_results %>% write_parquet(here::here("04-models","player_out_results.parquet"))
```


```{r}

player_out_results <- read_parquet(here::here("04-models","player_out_results.parquet"))
