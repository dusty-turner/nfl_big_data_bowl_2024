...
test_joined_with_preds <- 
  read_rds(here::here("04-models", "test_joined_with_preds.rds")) |> 
  select(game_id, play_id, display_name, frame_id, final_predictions, binary_predictions) |> 
  left_join(week_1) |> 
...
animated_plot2 <-
week_1 |>  filter(game_idplay_id == stub) |>
  left_join(test_joined_with_preds |> filter(game_idplay_id == stub)) |> 
...
week_1 |> 
  ...
  left_join(test_joined_with_preds |> filter(game_idplay_id == "20220908001030")) |> 
  relocate(final_predictions) |> 
  filter(frame_id <= max(frame_id)-4) |> 
  mutate(final_predictions = ifelse(club == defensive_team & is.na(final_predictions), 0, final_predictions)) |> 
  group_by(frame_id) |> 
...
# nn_briar <- 
week_1 |> select(game_id, play_id, display_name, frame_id, position) |> 
  # select(tackle, jersey_number) |> print(n = Inf)
  left_join(test_joined_with_preds) |> 
